...
const (
	EmployeeStatusDisabled EmployeeStatus = 0
	EmployeeStatusEnabled  EmployeeStatus = 1
)
...
type Employee struct {
	...
	IDNumber   string         `gorm:"type:varchar(18);not null;comment:身份证号" json:"idNumber"`
	Status     EmployeeStatus `gorm:"not null;default:1;comment:状态 0:禁用，1:启用" json:"status"`
	CreateTime time.Time      `gorm:"type:datetime;comment:创建时间" json:"createTime"`
	UpdateTime time.Time      `gorm:"type:datetime;comment:更新时间" json:"updateTime"`
	CreateUser int64          `gorm:"type:bigint;default:null;comment:创建人" json:"createUser"`
	UpdateUser int64          `gorm:"type:bigint;default:null;comment:修改人" json:"updateUser"`
}
